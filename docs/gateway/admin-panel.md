...
- **克隆**提供商——打开添加提供商弹窗，预填源提供商的设置作为模板
- **删除**提供商
...
当配置的服务方超过 6 个时，提供商区域顶部会自动出现搜索栏，可按名称、类型/shim 或 Base URL 实时过滤。

#### 网格 / 列表视图切换

提供商区域标题栏内有两个图标按钮，可在**网格**（卡片）和**列表**（紧凑行）布局之间切换。选择的布局会保存到 `localStorage` 中。
