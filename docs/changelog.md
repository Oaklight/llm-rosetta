...
- **`converters/base/` reorganized into helpers/ subpackage** ([#311](https://github.com/Oaklight/llm-rosetta/pull/311), [#310](https://github.com/Oaklight/llm-rosetta/issues/310)): Utility functions extracted from the flat `converters/base/` directory into `converters/base/helpers/`. Abstract base classes (the Ops pattern contract) stay at the top level; implementation utilities (`cache`, `schema`, `orphan_fix`, `tool_content`, `image_limit`, `tool_call_unwind`) move to `helpers/`. `tools.py` reduced from 428→185 lines (pure ABC). `helpers/__init__.py` re-exports public functions
