...
!!! note "谁需要看这页"
    写 shim 或运维网关的人。一般的推理/思考参数用法看[推理 / 思考参数](reasoning.md)就够了。

[推理 / 思考参数](reasoning.md)讲的是 LLM-Rosetta 怎么在 IR 层统一推理参数。这页记录的是**各家提供方实际怎么表现**——接受哪些 effort 值、禁用推理时会怎样、`thinking_type` 的具体含义，以及推理元数据在跨提供方往返中能不能保留。
