...
!!! note "目标读者"
    本页面面向 shim 作者和网关运维人员，需要了解各提供方推理行为的具体差异。一般推理/思考参数用法请参阅[推理 / 思考参数](reasoning.md)指南。

[推理 / 思考参数](reasoning.md)指南介绍了 LLM-Rosetta 如何通过 IR 层统一推理参数。本页面记录**各提供方的实际行为**——接受哪些 effort 值、如何处理禁用推理、`thinking_type` 的具体语义，以及推理元数据在跨提供方往返中的保留情况。

以下所有数据均来自对线上提供方 API 的实际探测，而非仅依赖文档。详见[探测方法](#探测方法)。

!!! tip "Shim 配置参考"
    每个提供方的 shim YAML 编码了本页记录的行为。shim 架构和 `provider.yaml` 格式详见[提供方 Shim](shims.md)。

## Effort 值接受情况

各提供方对 `reasoning_effort` / `output_config.effort` 值的接受情况。未列出的值会返回 400 错误，除非另有说明。

### 官方上游 API

2026-06-10 使用真实 API key 对官方端点探测（[#185](https://github.com/Oaklight/llm-rosetta/issues/185#issuecomment-4667473346)）。

| 提供方 | 端点 | `none` | `minimal` | `low` | `medium` | `high` | `xhigh` | `max` |
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
...
Argo 代理请求到上游提供方，但会进行自己的参数校验。Anthropic 端点使用 `output_config.effort`：
...
各提供方禁用推理的方式。shim 的 `disabled` 字段控制 LLM-Rosetta 使用哪种策略。

| 提供方 | 策略 | Shim `disabled` | 行为 |
|---|---|---|---|
...
!!! warning "Volcengine 和 DeepSeek 拒绝 `none`"
    向 Volcengine 或 DeepSeek 发送 `reasoning_effort: "none"` 会返回 400 错误。要在这些提供方上禁用推理，需使用 `thinking.type: "disabled"`——shim 的 `disabled: thinking_disabled` 策略会在 IR 模式为 `disabled` 时自动处理。

## Thinking Type

提供方使用 `thinking.type = "enabled"`（需要 `budget_tokens`）还是 `"adaptive"`（模型自行决定）。部分提供方和模型只接受其中一种。

### 提供方级默认值

| 提供方 | `thinking_type` | 说明 |
|---|---|---|
...
!!! info "opus47 shim 覆盖"
    `argo--anthropic` shim 在提供方级声明 `thinking_type: enabled`，同时为 `claudeopus47` 设置 `model_overrides` 覆盖为 `thinking_type: adaptive`。参见 [PR #256](https://github.com/Oaklight/llm-rosetta/pull/256)。

### 自动回退

当 shim 声明 `thinking_type: enabled` 但请求中没有 `budget_tokens`（Anthropic 的 `type: "enabled"` 必须提供此字段）时，converter 会自动回退为 `type: "adaptive"`，避免生成无效请求体，无需客户端了解提供方约束。

## 提供方元数据往返

推理块携带的提供方特定元数据需要在跨提供方转换中保留。LLM-Rosetta 通过内容块上的 IR `provider_metadata` 字段保留这些数据。
...
在提供方之间路由时（如 Anthropic 客户端 → Google 后端），来源提供方的签名必须保留，以确保：

1. 来源客户端可以使用有效签名重放对话历史
2. 目标提供方收到其自身格式的签名（如适用）
...
校验签名的提供方（如 Argo 的 Anthropic 端点）会拒绝这些块：
...
| 值 | 行为 |
|---|---|
| `as_is`（默认） | 原样转发未签名推理块，由目标提供方决定接受或拒绝。 |
...
reasoning:
  ...
  effort_map:                  # IR effort → 提供方 effort 字符串
    minimal: low
    low: low
    medium: medium
    high: high
...
| `effort_map` | map | *(恒等映射)* | IR effort 级别到提供方特定字符串的映射。未映射的级别会被丢弃并发出警告。 |
| `model_overrides` | map | *(无)* | 逐模型覆盖上述字段。键为 shim `model_id_field` 指定的上游模型 ID。 |

### 提供方 Shim 汇总

| 提供方 | `disabled` | `effort_field` | `thinking_type` | `max_effort` | `unsigned_reasoning_blocks` |
|---|---|---|---|---|---|
...
本页面所有行为数据均通过向线上提供方端点发送真实 API 请求并观察接受/拒绝响应获得。这种方法能发现提供方文档可能未覆盖的未记录约束。
...
提供方文档与实际 API 行为并不总是一致。Argo 探测中的典型案例：
