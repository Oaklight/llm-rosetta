...
这里记录的是把 LLM CLI 工具通过格式转换代理（如 [argo-proxy](https://github.com/Oaklight/argo-proxy) + LLM-Rosetta）路由时，实际跑出来的**兼容性问题**。每个问题都是在真实 CLI 会话中发现、然后在转换器里修掉的。
...
!!! info "方法论"
    下面的问题全是跑出来的，不是读 spec 读出来的——真实 CLI 会话里观察到 400 报错、数据静默丢失或行为异常后，逐个修复。对做跨提供方代理或格式转换器的人来说，这是一份经过验证的参考。
