...
!!! note "目标读者"
    本页面面向在多轮应用中嵌入库或扩展网关桥接行为的开发者。如果你只是直接使用网关，跨轮次状态由 `TurnBridge` 自动处理——你不需要阅读此页面。

当网关在两种不同 API 格式之间代理时，每个 HTTP 轮次都运行独立的转换 pipeline，拥有各自的 `ConversionContext`。存放在 context 中的 provider 特定元数据会在每轮结束后被销毁。本页说明跨轮次桥接状态的两层架构。
