...
class TestConverterCacheHintRoundTrip:
    ...
    def test_system_string_no_change(self):
        """Plain string system instruction stays a string (no regression)."""
        provider_request = {
            "model": "claude-sonnet-4-20250514",
            "max_tokens": 1024,
    ...
    def test_system_blocks_without_cache_control_stays_string(self):
        """System blocks without cache_control use system_instruction string."""
        provider_request = {
            "model": "claude-sonnet-4-20250514",
            "max_tokens": 1024,
            "system": [
        ...
        ir = self.converter.request_from_provider(provider_request)
        # No system message inserted when no cache_hint present
        system_msgs = [
            m for m in ir.get("messages", [])
            if isinstance(m, dict) and m.get("role") == "system"
        ]
        assert len(system_msgs) == 0
        assert ir["system_instruction"] == "You are helpful. Be concise."
